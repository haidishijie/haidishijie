...
/**
 * Vite plugin that mounts the TeslaMate Express API on the dev server.
 * No need for a separate backend process.
 */
function teslaMateApiPlugin(): Plugin {
  const pool = new Pool({
    host: '192.168.28.15',
    port: 5432,
    user: 'teslamate',
  ...
  api.get('/api/health', (_req, res) => res.json({ status: 'ok', timestamp: new Date().toISOString() }));

  api.get('/api/car', async (_req, res) => {
    try {
      const cars = await q(`
  ...
  api.get('/api/drives', async (req, res) => {
    try {
  ...
  api.get('/api/drives/:id', async (req, res) => {
    try {
  ...
  api.get('/api/drives/:id/positions', async (req, res) => {
    try {
  ...
  api.get('/api/charges', async (req, res) => {
    try {
      const limit = Math.min(100, Math.max(1, parseInt((req.query.limit as string) || '20', 10)));
      const offset = Math.max(0, parseInt((req.query.offset as string) || '0', 10));
  ...
  api.get('/api/charges/:id', async (req, res) => {
    try {
  ...
  api.get('/api/stats/weekly', async (_req, res) => {
    try {
  ...
  api.get('/api/stats/monthly', async (_req, res) => {
    try {
  ...
  api.get('/api/stats/summary', async (_req, res) => {
    try {
  ...
  api.get('/api/stats/mileage', async (req, res) => {
    try {
  ...
  api.get('/api/stats/efficiency', async (req, res) => {
    try {
      const limit = Math.min(100, Math.max(1, parseInt((req.query.limit as string) || '20', 10)));
  ...
  api.get('/api/stats/locations', async (_req, res) => {
    try {
  ...
  api.get('/api/commute/compare', async (req, res) => {
    try {
      ...
      const getDrive = async (id: number) => {
        const drive = await q1(`
          SELECT d.*, sa1.name AS start_address, sa2.name AS end_address,
            sg1.name AS start_geofence, sg2.name AS end_geofence
          FROM drives d
          LEFT JOIN addresses sa1 ON d.start_address_id = sa1.id
          LEFT JOIN addresses sa2 ON d.end_address_id = sa2.id
        ...
        `, [id]);
        const positions = await q(`
          SELECT latitude, longitude, speed, power, battery_level, date
          FROM positions WHERE drive_id = $1 ORDER BY date ASC
        `, [id]);
      ...
      const buildSegments = (positions: any[]) => {
        ...
        for (let i = 0; i < positions.length - 1; i += segSize) {
          ...
          const startTime = new Date(seg[0].date).getTime();
          const endTime = new Date(seg[seg.length - 1].date).getTime();
      ...
      const buildKPIs = (infoA: any, infoB: any) => {
        const kpis: any[] = [];
        const fmt = (v: number) => v.toString();
        if (infoA.duration < infoB.duration) kpis.push({ metric: 'Total Duration', valueA: fmt(infoA.duration), valueB: fmt(infoB.duration), winner: 'A' });
        else kpis.push({ metric: 'Total Duration', valueA: fmt(infoA.duration), valueB: fmt(infoB.duration), winner: 'B' });
        if (infoA.avgSpeed > infoB.avgSpeed) kpis.push({ metric: 'Avg Speed', valueA: fmt(infoA.avgSpeed), valueB: fmt(infoB.avgSpeed), winner: 'A' });
        else kpis.push({ metric: 'Avg Speed', valueA: fmt(infoA.avgSpeed), valueB: fmt(infoB.avgSpeed), winner: 'B' });
        if (infoA.maxSpeed > infoB.maxSpeed) kpis.push({ metric: 'Max Speed', valueA: fmt(infoA.maxSpeed), valueB: fmt(infoB.maxSpeed), winner: 'A' });
        else kpis.push({ metric: 'Max Speed', valueA: fmt(infoA.maxSpeed), valueB: fmt(infoB.maxSpeed), winner: 'B' });
        return kpis;
      };
...
export default defineConfig({
  plugins: [react(), teslaMateApiPlugin()],
  server: {
    host: true,
    port: 5173,
  },
